...
data class BackupDocument(val schemaVersion: Int = 1, val exportedAt: Long, val tasks: List<TaskItem>, val goals: List<GoalItem>, val events: List<TaskEvent>, val sessions: List<FocusSession>, val intervals: List<FocusInterval>)
...
object BackupCodec {
    ...
    fun validate(b: BackupDocument) {
        require(b.schemaVersion == 1 && b.exportedAt > 0)
        ...
        checkIds(b.tasks.map { it.id }); checkIds(b.goals.map { it.id }); checkIds(b.events.map { it.id }); checkIds(b.sessions.map { it.id }); checkIds(b.intervals.map { it.id })
        val tasks = b.tasks.map { it.id }.toSet(); val goals = b.goals.map { it.id }.toSet(); val sessions = b.sessions.associateBy { it.id }
        b.tasks.forEach { require(!it.title.isNullOrBlank() && it.note != null && it.status in listOf("TODO", "IN_PROGRESS", "DONE") && it.priority in 0..2 && TaskRules.validDate(it.plannedDate) && TaskRules.validDate(it.dueDate) && (it.goalId == null || it.goalId in goals) && (it.estimate == null || it.estimate in 1..10000)) }
        b.goals.forEach { require(!it.title.isNullOrBlank() && it.note != null && it.status in listOf("ACTIVE", "COMPLETED", "ARCHIVED") && TaskRules.validDate(it.dueDate)) }
...
class BackupService(private val context: Context, private val repo: Repository) {
    ...
    private suspend fun document(): BackupDocument {
        ...
        return BackupDocument(exportedAt = repo.clock.wall(), tasks = dao.tasks(), goals = dao.goals(), events = dao.events(), sessions = dao.sessions(), intervals = dao.intervals())
    }
    ...
    suspend fun restore(b: BackupDocument) = withContext(Dispatchers.IO) {
        ...
        repo.mutex.withLock {
            repo.database.withTransaction {
                ...
                val dao = repo.dao
                dao.clearTimer(); dao.clearIntervals(); dao.clearSessions(); dao.clearEvents(); dao.clearTasks(); dao.clearGoals()
